...
class SideMenu: UIView, UITableViewDelegate, UITableViewDataSource {
    ...
    func toggleMenu(open: Bool) {
        ...
        UIView.animate(withDuration: 0.2) { 
            self.backgroungView.alpha = open ? 0.5 : 0
        }
    ...
    func setupMenuView() {
        ...
        backgroungView = UIView(frame: parentViewController.view.bounds)
        backgroungView.backgroundColor = UIColor.lightGray
        backgroungView.alpha = 0
        parentViewController.view.insertSubview(backgroungView, belowSubview: self)
